...
flowchart TD
    ...
    subgraph home["Home Network"]
        workstation["Workstation - Fedora<br/>Daily driver + KVM host<br/>Practice VMs"]
        server["Server - Arch Linux<br/>Bare-metal + KVM host<br/>Docker / Portainer, Media stack, Immich"]
        oob["Out-of-Band Mgmt<br/>KVM-over-IP + power-button controller"]
    end

    subgraph cloud["Cloud (ARM)"]
        cloudnode["Cloud Node - Linux<br/>Self-hosted WireGuard VPN<br/>Containerized services"]
    end

    cf -->|named SSH via Tunnel| workstation
    cf -->|named SSH via Tunnel| server
    cf -->|named SSH via Tunnel| cloudnode
    oob -.->|hardware-level access| server
    workstation -->|SSH admin| server
    workstation -->|SSH admin| cloudnode
```
...
### Workstation — Fedora (daily driver + hypervisor)
